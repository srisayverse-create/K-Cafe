...
const CafeLogo = ({ size = "lg", className = "" }: { size?: "sm" | "md" | "lg", className?: string }) => {
  ...
  const fontSize = size === "lg" ? "text-6xl" : size === "md" ? "text-3xl" : "text-xl";
  
  return (
    <div className={`relative flex items-center justify-center ${dimensions} ${className}`}>
      {/* Outer Ring */}
      <div className="absolute inset-0 rounded-full border-2 border-current opacity-40 shadow-inner" />
      {/* Inner Ring */}
      <div className="absolute inset-2 rounded-full border border-current opacity-60" />
      
      {/* Circular Text (Approximated with CSS) */}
      <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
        <svg viewBox="0 0 100 100" className="w-full h-full animate-[spin_20s_linear_infinite] opacity-80">
          <path id="circlePath" d="M 50, 50 m -37, 0 a 37,37 0 1,1 74,0 a 37,37 0 1,1 -74,0" fill="transparent" />
          <text className="fill-current text-[7px] font-black tracking-[0.2em] uppercase">
            <textPath xlinkHref="#circlePath" startOffset="0%"> KUMBAKONAM CAFE • AUTHENTIC VEG EATERY • </textPath>
          </text>
        </svg>
      </div>

      <div className="relative z-10 flex flex-col items-center justify-center">
        <span className={`font-brand font-black text-current leading-none ${fontSize}`}>K</span>
      </div>
      
      {/* Trademark marker for large logo */}
      {size === "lg" && (
        <div className="absolute bottom-6 right-0 w-6 h-6 rounded-full border border-current flex items-center justify-center text-[8px] font-bold opacity-60">R</div>
      )}
    </div>
  );
};
...
const AboutTimeline = ({ theme }: { theme: string }) => {
  ...
  const galleryImages = [
    { 
      url: "https://images.unsplash.com/photo-1541167760496-162955ed8a9f?q=80&w=2670&auto=format&fit=crop", 
                ...
                <div className="relative z-10 flex flex-col lg:flex-row gap-12">
                  ...
                  <div className="lg:w-1/3">
                    ...
                    >
                      ...
                      <div className="w-16 h-16 rounded-full bg-white/20 backdrop-blur-md flex items-center justify-center z-20 group-hover:scale-110 transition-transform">
                        <Play size={24} className="text-white fill-white ml-1" />
                      </div>
                      <img src="https://images.unsplash.com/photo-1541167760496-162955ed8a9f?q=80&w=2670&auto=format&fit=crop" className="w-full h-full object-cover grayscale brightness-50" alt="Video about Kumbakonam Cafe story" />
                      <div className="absolute bottom-4 left-4 z-20 text-[10px] font-bold text-white/50 uppercase tracking-widest">Featured Story Video</div>
                    </motion.div>
                  </div>
                </div>
              </div>
...
const FloatingDeliveryButton = ({ onClick }: { onClick: () => void }) => {
  return (
    <motion.button
      initial={{ opacity: 0, scale: 0.8 }}
      ...
      whileTap={{ scale: 0.9 }}
      onClick={onClick}
      className="fixed bottom-28 right-10 z-50 w-16 h-16 bg-[#1e9ab0] text-white rounded-2xl flex items-center justify-center neo-raised shadow-glow transition-all cursor-pointer"
...
const CategoryModal = ({ 
  ...
  }, [isOpen, initialIndex, category]);

  if (!category) return null;

  const currentItem = category.items[activeIndex];

  const nextDish = () => {
    setActiveIndex((prev) => (prev + 1) % category.items.length);
  };

  const prevDish = () => {
    setActiveIndex((prev) => (prev - 1 + category.items.length) % category.items.length);
  };

  // Safe renaming of Shabiya to Shabiya11
  const cleanDescription = (desc?: string) => {
    if (!desc) return "Our signature recipe made with authentic ingredients sourced directly from the heart of Tamil Nadu. A pure vegetarian delight.";
    return desc.replace(/Shabiya/g, "Shabiya11");
  };

  const cleanBranchInfo = (desc?: string) => {
    if (desc?.toLowerCase().includes("exclusively at")) {
      const branchPart = desc.split(/exclusively at /i)[1];
      return branchPart.replace(/Shabiya/g, "Shabiya11").replace(/ branch/i, " Branch");
    }
    return "Hamdan, Khalidiyah, Musaffah, Shabiya11";
  };
    ...
    <AnimatePresence>
      {isOpen && (
        <motion.div
          ...
          animate={{ opacity: 1 }}
          exit={{ opacity: 0 }}
          className="fixed inset-0 z-[500] flex items-center justify-center p-4 lg:p-10 bg-black/85 backdrop-blur-md"
          onClick={onClose}
        >
          {/* Prev Arrow */}
          <motion.button
          ...
          <motion.div
            ...
            className="w-full max-w-6xl bg-brand-lightest dark:bg-dark-surface rounded-[3rem] overflow-hidden shadow-2xl relative max-h-[90vh] flex flex-col lg:flex-row border border-brand/5 dark:border-brand-lightest/5"
            onClick={(e) => e.stopPropagation()}
          >
            {/* Left Column: Image Area */}
            <div className="w-full lg:w-[45%] bg-[#0c0c0c]/10 dark:bg-black/20 p-8 lg:p-12 flex flex-col gap-6 items-center justify-center min-h-[300px] lg:min-h-0 relative">
              <div className="w-full h-full aspect-square rounded-[2rem] overflow-hidden relative shadow-inner">
                {currentItem?.image ? (
                  ...
                  <div className="w-full h-full bg-[#1e9ab0]/10 flex flex-col items-center justify-center text-center p-8 gap-4 border border-[#1e9ab0]/20 rounded-[2rem]">
                    <Coffee size={64} className="text-[#1e9ab0] animate-pulse" />
                    <span className="text-xl font-brand font-bold text-[#1e9ab0] uppercase tracking-widest">Kumbakonam Cafe</span>
                    <span className="text-[10px] font-sans font-medium text-cafe-dark/40 dark:text-white/40 uppercase tracking-widest">Live Kitchen Fresh</span>
                  </div>
                )}
              </div>

              {/* Progress text indicator */}
              <div className="text-xs uppercase font-black tracking-widest text-cafe-dark/40 dark:text-white/40">
                Item {activeIndex + 1} of {category.items.length}
              </div>
            </div>

            {/* Right Column: Info & Navigation List */}
            <div className="w-full lg:w-[55%] p-8 lg:p-12 flex flex-col bg-brand-lightest dark:bg-dark-surface transition-colors relative overflow-y-auto">
              {/* Top Banner details */}
              <div className="mb-6">
                <div className="flex items-center gap-3 mb-2">
                  <Sparkles size={16} className="text-[#1e9ab0] animate-pulse" />
                  <span className="text-[10px] font-black uppercase tracking-[0.4em] text-[#1e9ab0]">
                    {category.title}
                  </span>
                </div>
                ...
                {/* Timing & Branch details */}
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 py-4 border-t border-b border-brand/5 dark:border-brand-lightest/5">
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 bg-brand/5 rounded-full flex items-center justify-center text-brand shrink-0">
                      <Clock size={16} />
                    </div>
                    <div className="flex flex-col">
                      <span className="text-[10px] font-black uppercase tracking-widest text-[#1e9ab0]/60">Serving Hours</span>
                      <span className="text-cafe-muted dark:text-dark-muted text-sm font-medium italic">
                        {category.timing || "All Day"}
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex flex-col">
                      <span className="text-[10px] font-black uppercase tracking-widest text-[#1e9ab0]/60">Available At</span>
                      <span className="text-cafe-muted dark:text-dark-muted text-sm font-medium italic">
                        {cleanBranchInfo(currentItem?.description)}
                      </span>
                    </div>
                  </div>
                </div>
              ...
              {/* Grid / List of other dishes in category */}
              <div className="mt-auto border-t border-brand/5 dark:border-brand-lightest/5 pt-6">
                <h4 className="text-[10px] font-black uppercase tracking-widest text-cafe-dark/40 dark:text-white/40 mb-4 block">
                  Other Dishes in Category
                </h4>
                <div className="flex gap-4 overflow-x-auto pb-4 max-h-44 scrollbar-thin">
                  {category.items.map((item, idx) => {
                    ...
                    return (
                      <button
                        ...
                        className={`flex-shrink-0 flex items-center gap-3 p-3 rounded-2xl border transition-all text-left outline-none ${
                          isActive 
                            ? 'border-[#1e9ab0] bg-[#1e9ab0]/5 text-brand font-medium' 
                            : 'border-brand/10 dark:border-brand-lightest/10 bg-white/5 hover:bg-[#1e9ab0]/5'
                        }`}
                      >
                        {item.image ? (
                        ...
                        <div className="max-w-[120px]">
                          <p className="text-[11px] uppercase tracking-wide truncate font-bold text-cafe-dark dark:text-dark-text">
                            {item.name}
                          </p>
                        </div>
                      </button>
            ...
            <button
              onClick={onClose}
              className="absolute top-6 right-6 text-cafe-dark/30 hover:text-[#1e9ab0] dark:text-white/30 dark:hover:text-[#1e9ab0] transition-colors p-2"
      ...
      <div className="max-w-7xl mx-auto relative">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 lg:gap-16">
          ...
          <div className="lg:col-span-6 space-y-10">
            ...
            >
              <div className="absolute inset-0 bg-black/40 z-10 pointer-events-none group-hover:bg-black/20 transition-colors" />
              <video 
                autoPlay 
                muted 
                loop 
                ...
                className="w-full h-full object-cover"
              >
                <source src="https://assets.mixkit.co/videos/preview/mixkit-chef-preparing-a-dish-with-herbs-and-spices-41314-large.mp4" type="video/mp4" />
              </video>
              ...
              <div className="absolute bottom-8 left-8 z-30">
                <div className="px-4 py-1.5 rounded-full bg-brand/80 backdrop-blur-md border border-white/20 text-[10px] font-bold text-white uppercase tracking-widest mb-2">Exclusive Review</div>
                <h3 className="text-2xl font-display font-bold italic text-white">"A Spiced Masterpiece"</h3>
              </div>
            </motion.div>
            ...
            >
              ...
              <video 
                ...
                className="w-full h-full object-cover"
              >
                <source src="https://assets.mixkit.co/videos/preview/mixkit-hot-soup-in-the-kitchen-of-a-restaurant-41316-large.mp4" type="video/mp4" />
              </video>
              <div className="absolute inset-0 flex items-center justify-center z-20">
              ...
              <div className="absolute bottom-6 left-6 z-30">
                <span className="text-[9px] font-black uppercase tracking-widest text-white/50">Kitchen Chronicles</span>
              </div>
            </motion.div>
          </div>
  ...
  return (
    <nav className={`fixed top-0 left-0 right-0 z-[100] transition-all duration-700 ${isScrolled ? 'top-0' : 'top-4 lg:top-8'}`} role="navigation">
      <div className="max-w-[1600px] mx-auto px-4 lg:px-6 relative">
          <div className={`flex items-center justify-between px-6 lg:px-8 py-3 lg:py-4 rounded-[1.5rem] border transition-all duration-700 ${isScrolled ? 'glass bg-white/90 dark:bg-dark-surface/90 border-[#1e9ab0]/5 dark:border-white/5 shadow-xl backdrop-blur-xl rounded-none border-t-2 border-t-[#1e9ab0]' : 'bg-transparent border-transparent'}`}>
          ...
          <div className="hidden lg:flex items-center gap-10">
            {navLinks.map((link) => (
              <a 
                key={link.name} 
                href={link.href} 
                className="text-[11px] font-black uppercase tracking-[0.4em] text-[#fff628] hover:text-[#1e9ab0] transition-all relative group"
              >
                {link.name}
              </a>
            ))}
          ...
          {/* Center Brand - Adjusted position to be clear of buttons */}
          <div className="absolute left-1/2 -translate-x-1/2 top-1/2 -translate-y-1/2">
             <a href="#home" className="flex flex-col items-center gap-2 group" aria-label="Kumbakonam Cafe Home">
               <CafeLogo size="sm" className="group-hover:scale-110 transition-transform text-brand" />
               <div className="flex flex-col items-center leading-none pointer-events-none">
                 <span className="font-brand font-black text-xl md:text-3xl tracking-tight text-cafe-dark dark:text-dark-text transition-colors">Kumbakonam Cafe LLC</span>
                 <span className="text-[7px] md:text-[8px] font-black tracking-[0.6em] text-brand -mt-1 opacity-100 italic">Authentic Veg Eatery</span>
                 <span className="text-[6px] md:text-[7px] font-black tracking-[0.5em] text-brand -mt-1 opacity-80">Experience the Goodness</span>
               </div>
             </a>
          </div>
          ...
          <div className="flex items-center gap-4">
            <div className="hidden lg:flex items-center gap-4">
                <div className="delivery-icon" onClick={onDeliveryPartnersClick} title="Order Now (Delivery)">
                  <Truck size={24} className="text-brand" />
                </div>
            </div>
            {/* Mobile Menu Toggle handled below in the original code... */}

            <button 
              onClick={() => setMobileMenuOpen(!mobileMenuOpen)} 
              className="lg:hidden text-cafe-dark dark:text-dark-text w-10 h-10 flex items-center justify-center bg-brand/5 dark:bg-brand-lightest/5 rounded-full"
               ...
               <button onClick={() => setMobileMenuOpen(false)} className="absolute top-10 right-10 text-black/50 dark:text-dark-text/50 hover:text-[#1e9ab0] transition-colors"><X size={40} /></button>
                ...
                {['HOME', 'MENU', 'ABOUT', 'BRANCHES', 'CONTACT'].map((item, i) => (
                  <motion.a 
                    ...
                    className="text-6xl font-display font-bold italic text-[#fff628]/20 dark:text-white/20 hover:text-[#fff628] transition-all hover:scale-110"
...
const DeliveryPartnersModal = ({ isOpen, onClose }: { isOpen: boolean, onClose: () => void, theme: string }) => {
  const [selectedLocation] = useState("uae");
                            ...
                            {DELIVERY_CONFIG.locations[selectedLocation as keyof typeof DELIVERY_CONFIG.locations].partners.map((partner, idx) => (
                              <a
                                ...
                                className={`flex items-center gap-4 p-5 rounded-xl border-2 transition-all hover:-translate-y-1 shadow-sm hover:shadow-md
                                  ${partner.iconColor === 'talabat' ? 'border-[#FF5A00] bg-[#FF5A00]/5 hover:bg-[#FF5A00]/10 text-black' : 
                                    partner.iconColor === 'noon' ? 'border-[#FEE000] bg-[#FEE000]/10 hover:bg-[#FEE000]/20 text-black' : 
                                    partner.iconColor === 'smiles' ? 'border-[#E94359] bg-[#E94359]/5 hover:bg-[#E94359]/10 text-black' : 
                                    'border-[#47B749] bg-[#47B749]/5 hover:bg-[#47B749]/10 text-black'}`}
                              >
                                <div className="flex-1">
                                  <h4 className="font-bold text-xl leading-tight font-brand mb-1 text-black">{partner.name}</h4>
                                  <p className="text-sm text-gray-500 font-sans">{partner.description}</p>
                                </div>
...
const allItems = useMemo(() => {
  const items = MENU_DATA.flatMap(cat => cat.items)
    .filter(item => SIGNATURE_DISHES.includes(item.name))
  ...
  return (
    <section id="home" className="relative min-h-screen flex items-center pt-32 lg:pt-20 overflow-hidden transition-colors duration-1000">
      ...
      <div className="relative z-10 w-full max-w-[1600px] mx-auto px-6 lg:px-12 bg-transparent rounded-[4rem] lg:my-12 overflow-hidden grid grid-cols-12 gap-8 lg:gap-12 items-center">
        ...
        <div className="col-span-12 xl:col-span-7 flex flex-col items-start order-2 md:order-1 p-10 lg:p-20 relative z-10">
            <motion.div
              ...
              <div className="flex items-center gap-3 py-2.5 px-8 rounded-full border-2 border-[#1e9ab0]/30 w-fit mb-8 lg:mb-12 mx-auto lg:mx-0 shadow-sm">
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-[11px] font-brand font-black uppercase tracking-[0.5em] text-[#1e9ab0] bg-white px-3 py-2 rounded-full">WELCOME TO KUMBAKONAM CAFE</span>
                           ...
                           <h1 className="text-[clamp(3rem,8vw,6.5rem)] leading-[0.95] font-display tracking-tighter text-brand-lightest mb-10 lg:mb-12 text-center lg:text-left transition-colors flex flex-col lg:items-start gap-6 lg:gap-10 justify-center lg:justify-start">
                <motion.div 
                  initial={{ opacity: 0, scale: 0.8, rotate: -20 }}
                  animate={{ opacity: 1, scale: 1, rotate: 0 }}
                  className="shrink-0 flex items-center justify-center opacity-100 mb-4"
                >
                  <CafeLogo size="lg" className="text-white" />
                </motion.div>
                <ExperienceGoodness className="text-white" />
               </h1>
            ...
            >
              ...
              >
               ...
               <motion.button 
                 ...
                 className="flex-1 sm:flex-none px-12 py-7 lg:py-8 bg-[#fff628] text-black font-black text-[11px] lg:text-[12px] uppercase tracking-[0.4em] flex items-center justify-center gap-4 rounded-full shadow-2xl active:scale-95 border border-black/5 hover:bg-white transition-all"
               >
                 <Download size={18} strokeWidth={2.5} /> DOWNLOAD MENU
               </motion.button>
             </motion.div>
        ...
        <div className="col-span-12 xl:col-span-5 flex justify-center xl:justify-end order-1 md:order-2 py-10 lg:py-20 lg:pr-12">
           <AnimatePresence mode="wait">
             <motion.div
               key={slideIndex}
      ...
      <div className="p-8">
        ...
        <div className="mt-8 flex items-center justify-between border-t border-brand/5 dark:border-brand-lightest/5 pt-6">
          <div className="flex items-center gap-2">
            ...
            <span className="text-[10px] font-black uppercase tracking-widest text-black dark:text-white">Live Kitchen Fresh</span>
          </div>
          <button 
            onClick={() => onQuickView(item)}
            className="text-[10px] font-black uppercase tracking-[0.2em] text-black dark:text-white flex items-center gap-2 group-hover:gap-4 transition-all"
...
const MenuSection = ({ theme, searchQuery, setSearchQuery, onCategorySelect }: { 
  ...
  onCategorySelect: (category: MenuCategory, initialIndex: number) => void
}) => {
  ...
  // Filter out weekend specials
  const visibleCategories = useMemo(() => {
    return MENU_DATA.filter(cat => cat.id !== 'weekend');
  }, []);

  return (
    <section id="menu" ref={sectionRef} className="py-24 lg:py-40 px-6 relative overflow-hidden bg-[#0c0c0c] text-white">
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        <div className="text-center mb-16 lg:mb-24">
          ...
          >
            Experience Our <span className="text-[#fff628]">Menu</span>
          </motion.h2>
          <motion.p 
            initial={{ opacity: 0 }}
          ...
          {visibleCategories.map((category, idx) => {
            ...
            return (
              <motion.div
                ...
                transition={{ delay: idx * 0.05, scale: { duration: 0.4, ease: [0.22,1,0.36,1] } }}
                viewport={{ once: true }}
                onClick={() => { 
              ...
              >
                ...
                <div className="absolute inset-0 z-20 p-8 flex flex-col justify-end">
                  <div className="absolute inset-x-0 bottom-0 h-[80%] bg-gradient-to-t from-black via-black/50 to-transparent opacity-80" />

                  <div className="relative z-30 transition-transform duration-500 group-hover:-translate-y-4">
                    {/* Timing subtitle */}
                    <div className="mb-4">
                      <span className="text-[10px] font-black uppercase tracking-[0.4em] text-[#1e9ab0] drop-shadow-[0_2px_4px_rgba(0,0,0,0.5)]">
                        {category.timing || 'All Day'}
                      </span>
                    </div>
                    
                    {/* Title */}
                    <h3 className="text-2xl lg:text-3xl font-display italic font-bold mb-3 drop-shadow-[0_4px_12px_rgba(0,0,0,0.8)] text-white group-hover:text-[#1e9ab0] transition-colors leading-[1.1]">
                      {category.title}
                    </h3>

                    {/* Preview list of dishes */}
...
const BranchModal = ({ branch, isOpen, onClose }: { branch: any, isOpen: boolean, onClose: () => void }) => {
  const [slideIndex, setSlideIndex] = useState(0);
    ...
    <AnimatePresence>
      {isOpen && (
        <motion.div 
          ...
          animate={{ opacity: 1 }}
          exit={{ opacity: 0 }}
          className="fixed inset-0 z-[250] flex items-center justify-center p-4 lg:p-10 bg-black/80 backdrop-blur-md"
          onClick={onClose}
        >
          <motion.div 
            initial={{ scale: 0.95, opacity: 0 }}
            animate={{ scale: 1, opacity: 1 }}
            exit={{ scale: 0.95, opacity: 0 }}
            className="w-full max-w-7xl bg-white rounded-[2rem] overflow-hidden shadow-2xl relative max-h-[90vh] flex flex-col lg:flex-row"
            onClick={(e) => e.stopPropagation()}
          >
...
const BranchesSection = ({ theme }: { theme: string }) => {
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 lg:gap-6 min-h-[600px]">
          {BRANCHES.map((branch, idx) => (
            <motion.div
              ...
              className="relative group h-[500px] lg:h-[600px] rounded-[3.5rem] overflow-hidden cursor-pointer border border-white/5 bg-white/5 transition-all outline-none"
            >
              {/* Background Image with Overlay */}
              <div className="absolute inset-0 z-0">
                <AnimatePresence mode="wait">
                  <motion.img 
                    key={idx}
                    src={branch.outletImages?.[0] || "https://images.unsplash.com/photo-1555396273-367ea4eb4db5?q=80&w=2674&auto=format&fit=crop"} 
                    className="w-full h-full object-cover transition-transform duration-1000 group-hover:scale-110 grayscale group-hover:grayscale-0"
                    alt={branch.name} 
                  />
                </AnimatePresence>
      ...
      <div className="max-w-[1400px] mx-auto px-6 relative z-10">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12 lg:gap-16 mb-16">
          ...
          <div className="text-center md:text-left">
            ...
            <button className="px-10 py-3.5 bg-[#2c8496] text-white font-display font-bold uppercase tracking-widest text-[13px] rounded-md mb-12 inline-flex items-center justify-center hover:bg-black transition-colors shadow-md">
               DOWNLOAD MENU
            </button>
            <h4 className="text-2xl font-display font-medium text-black mb-8">Branches</h4>
            <ul className="space-y-6">
              {['Hamdan', 'Khalidiyah', 'Musaffah'].map((branch) => (
                <li key={branch}>
                  <a href="#branches" className="flex items-center justify-center md:justify-start gap-3 text-black/80 hover:text-[#1e9ab0] transition-colors italic text-lg group">
                    <ChevronRight size={16} className="text-[#1e9ab0] group-hover:translate-x-1 transition-transform" /> {branch}
                  </a>
                </li>
              ))}
            </ul>
